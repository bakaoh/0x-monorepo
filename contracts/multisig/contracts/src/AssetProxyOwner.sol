    ...
    MultiSigWalletWithTimeLock
{
        ...
        fullyConfirmed(transactionId)
    {
        ...
        // `destination` and `value` fields are ignored
        BatchTransaction memory batchTransaction = abi.decode(transaction.data, (BatchTransaction));

        // Ensure lengths of array properties are equal
        uint256 length = batchTransaction.data.length;
        require(
            length == batchTransaction.destinations.length && length == batchTransaction.values.length,
            "EQUAL_LENGTHS_REQUIRED"
        );
        ...
        for (uint i = 0; i != length; i++) {
            ...
            _assertValidFunctionCall(
                ...
                batchTransaction.data[i],
                batchTransaction.destinations[i]
            );
            // Call each function
            (bool didSucceed,) = batchTransaction.destinations[i].call.value(batchTransaction.values[i])(batchTransaction.data[i]);
